...
| `test` | Run tests | `bundle exec rake` (default core spec suite), or `bundle exec rspec <spec paths>` for focused tests |
...
For focused examples, pass relative paths below `spec/` to `.agents/bin/test`;
they must resolve inside that directory and be spec files or directories.
Without paths, it runs the default Rake core suite.
